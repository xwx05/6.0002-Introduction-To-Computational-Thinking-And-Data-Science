...
class RobotVisualization:
    def __init__(self, num_robots, width, height, is_furnished, delay = 0.2):
    ...
    def update(self, room, robots):
        ...
        for i in range(self.width):
            for j in range(self.height):
                x1, y1 = self._map_coords(i, j)
                x2, y2 = self._map_coords(i + 1, j + 1)
                if (not room.is_tile_cleaned(i, j) and not self.is_furnished) or (not room.is_tile_cleaned(i, j) and not room.is_tile_furnished(i, j)):
                    #get dirt amount
                ...
                elif self.is_furnished and room.is_tile_furnished(i, j):
