...
class RectangularRoom(object):
    ...
    def __init__(self, width, height, dirt_amount):
        ...
        self.width = width
        self.height = height
        self.dirt_amount = {(x, y): dirt_amount for x in range(width) for y in range(height)}
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        self.dirt_amount[(x, y)] = max(0, self.dirt_amount[(x, y)] - capacity)  # capacity是每步的清理量
    ...
    def get_num_cleaned_tiles(self):
        ...
        return sum(1 for v in self.dirt_amount.values() if v == 0)
...
class Robot(object):
    ...
    def __init__(self, room, speed, capacity):
        ...
        self.position = Position(random.random() * self.room.width, random.random() * self.room.height)  # 坐标位置为float
...
class FurnishedRoom(RectangularRoom):
    ...
    def is_position_furnished(self, pos):
        ...
        """
        x, y = int(pos.get_x()), int(pos.get_y())  # int()函数会直接截断小数部分，由于本题坐标均为正数，所以可以使用int()
...
class FaultyRobot(Robot):
    ...
    """
    p = 0.15  # 出错（不会清洁瓷砖并随机转换一个方向）的概率

    @staticmethod  # 静态方法，不需要创建实例，可以直接通过类名调用
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    dirt_amount: an int
    min_coverage: a float (0 <= min_coverage <= 1.0)  干净瓷砖占总瓷砖的比例
    ...
    for t in range(num_trials):
        ...
        anim = ps3_visualize.RobotVisualization(num_robots, width, height, True, 0.2)
        time_steps = 0

        while room.get_num_cleaned_tiles() / float(room.get_num_tiles()) < min_coverage:
            for robot in robots:
    ...
    return total_time_steps / float(num_trials)
...
#       of a 20x20 room?
# A：StandardRobot比FaultyRobot在20x20的房间中清洁80%的性能要好。
#
# 2) How does the performance of the two robot types compare when two of each
#       robot cleans 80% of rooms with dimensions 
#       10x30, 20x15, 25x12, and 50x6?
# A：StandardRobot在不同房间的整体表现较为接近，其中在10x30和20x15的房间中更好，而FaultyRobot在20x15的房间中明显表现更好。
#
