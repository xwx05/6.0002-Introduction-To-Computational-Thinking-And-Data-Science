...
def plotData(fileName):
    ...
    xVals = xVals * 9.81  # acc. due to gravity  F = mg
...
def fitData(fileName):
    ...
    xVals = pylab.array(xVals)
    yVals = pylab.array(yVals)
    xVals = xVals * 9.81  # get force F = mg
    pylab.plot(xVals, yVals, 'bo',
               label='Measured points')
    labelPlot()  # 画出原始数据
    a, b = pylab.polyfit(xVals, yVals, 1)  # 横坐标为力F，纵坐标为距离x
    estYVals = a * pylab.array(xVals) + b
    print('a =', a, 'b =', b)
    pylab.plot(xVals, estYVals, 'r',
               label='Linear fit, k = '
                     + str(round(1 / a, 5)))  # 胡克定律F = kx，所以本图中，k=1/a
...
def fitData1(fileName):
    ...
    estYVals = pylab.polyval(model, xVals)  # 使用polyval函数画图，呈现结果与fitData相同
    pylab.plot(xVals, estYVals, 'r',
               label='Linear fit, k = '
...
# pylab.legend()  # 二阶函数拟合效果较好
# pylab.show()


# # Compare models  两种模型之间的相对差异
...
def rSquared(observed, predicted):
    error = ((predicted - observed) ** 2).sum()  # 误差平方和
    meanError = error / len(observed)  # error/n
    return 1 - (meanError / numpy.var(observed))  # numpy.var(observed)即计算observed的总体方差1/n*(observed-mean)的平方和
